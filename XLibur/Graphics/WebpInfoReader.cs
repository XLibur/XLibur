...
internal sealed class WebpInfoReader : ImageInfoReader
{
    ...
    protected override XLPictureInfo ReadInfo(Stream stream)
    {
        ...
        if (chunkCode == ExtendedV8Code)
            return ReadExtendedInfo(stream);

        if (chunkCode == LossyVp8Code)
            return ReadLossyInfo(stream);

        if (chunkCode == LosslessVp8Code)
            return ReadLosslessInfo(stream);

        throw new ArgumentException("Invalid chunk for WebP file.");
    }

    private static XLPictureInfo ReadExtendedInfo(Stream stream)
    {
    ...
    private static XLPictureInfo ReadLossyInfo(Stream stream)
    {
        ...
        var width = GetLossySize(widthAndScale);

        var heightAndScale = stream.ReadU16LE();
        var height = GetLossySize(heightAndScale);

        return new XLPictureInfo(XLPictureFormat.Webp, new Size(width, height), Size.Empty, 72, 72);
    }

    private static int GetLossySize(ushort sizeAndScale)
    {
    ...
    private static XLPictureInfo ReadLosslessInfo(Stream stream)
    {
