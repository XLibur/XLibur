...
internal sealed class XLPivotTable : IXLPivotTable, ISheetListener
{
    ...
    {
        get => PivotCache;
        set => PivotCache = (XLPivotCache)value;
    }
    ...
    public string Name
    {
        ...
        set
        {
            ...
            if (!XLHelper.ValidateName("pivot table", value, oldname, Worksheet.PivotTables.Select(pvt => pvt.Name),
                    out string message))
    ...
    {
        UseCustomListsForSorting = value;
        return this;
    }
    ...
    {
        ShowPropertiesInTooltips = value;
        return this;
    }
...
#pragma warning restore S2376
    ...
    /// would renumber the copy's identically-positioned area too.
    /// </remarks>
    internal static bool RenumberDataFieldPositions(XLPivotArea area, uint removedPosition, uint remainingValueCount)
    {
    ...
    /// file shows what that looks like. The pivot areas are cloned (<see cref="XLPivotArea.Clone"/>),
    /// not shared: a conditional format's area can change after the copy, when a value is removed from
    /// either table's <see cref="DataFields"/> renumbers its 'data' field references
    /// (<see cref="RemoveValueFromFormats"/>), and sharing the area would let a removal on one table
    /// renumber the other's format for a value it never lost (#585).
    /// </remarks>
    ...
    /// <summary>
    /// Add a field to a specific axis (page/row/col). Only modified <see cref="PivotFields"/>, doesn't modify
    ...
    /// <summary>
    /// Refresh cache fields after the cache has changed.
    /// </summary>
    internal void UpdateCacheFields(IReadOnlyList<string> oldFieldNames)
    {
        ...
        var keptFilterSourceNames = GetKeptNames(Filters.Fields.Select(x => (FieldIndex)x.Field).ToList(),
            oldFieldNames, newNames, includeValuesField);
    ...
    /// <summary>
    /// Is a field used by any axis (row, column, filter), but not data.
    /// </summary>
    ...
    internal int GetFieldIndex(XLPivotTableField field)
    {
        ...
        return index < 0 ? throw new ArgumentException($"Unable to find field '{field.Name}'.") : index;
    }
}
