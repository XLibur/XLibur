...
internal readonly struct XLRangeAddress : IXLRangeAddress, IEquatable<XLRangeAddress>
{
    ...
    {
        return new XLRangeAddress(
            new XLAddress(worksheet, range.FirstPoint.Row, range.FirstPoint.Column, fixedRow: false,
                fixedColumn: false),
    ...
    {
        return referenceStyle == XLReferenceStyle.R1C1
            ? ToStringFixed(referenceStyle, true)
            : ToStringRelative(includeSheet);
    }

    public override bool Equals(object? obj)
    {
        if (obj is not XLRangeAddress address)
        {
            return false;
        }
    ...
    internal XLRangeAddress Relative(in XLRangeAddress sourceRangeAddress, in XLRangeAddress targetRangeAddress)
    {
        ...
        (
            ...
            (
                sheet,
                FirstAddress.RowNumber - sourceRangeAddress.FirstAddress.RowNumber +
                targetRangeAddress.FirstAddress.RowNumber,
                FirstAddress.ColumnNumber - sourceRangeAddress.FirstAddress.ColumnNumber +
            ...
            (
                sheet,
                LastAddress.RowNumber - sourceRangeAddress.FirstAddress.RowNumber +
                targetRangeAddress.FirstAddress.RowNumber,
                LastAddress.ColumnNumber - sourceRangeAddress.FirstAddress.ColumnNumber +
                targetRangeAddress.FirstAddress.ColumnNumber,
                fixedRow: false,
                fixedColumn: false
            )
    ...
    internal XLRangeAddress Intersection(in XLRangeAddress otherRangeAddress)
    {
        ...
        var firstRow = Math.Max(thisRangeAddressNormalized.FirstAddress.RowNumber,
            otherRangeAddressNormalized.FirstAddress.RowNumber);
        var firstColumn = Math.Max(thisRangeAddressNormalized.FirstAddress.ColumnNumber,
            otherRangeAddressNormalized.FirstAddress.ColumnNumber);
        var lastRow = Math.Min(thisRangeAddressNormalized.LastAddress.RowNumber,
            otherRangeAddressNormalized.LastAddress.RowNumber);
        var lastColumn = Math.Min(thisRangeAddressNormalized.LastAddress.ColumnNumber,
