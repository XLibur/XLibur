...
- **The first edit after a load allocates about 45% less, and the dependency tree it builds keeps about 46% less memory.** After a load, the first edit builds the dependency tree of the calc engine for every formula in the workbook (#504). A full recalculation builds it too. On a workbook of 200,000 formulas, the build allocated 788 MB, and the tree kept 146 MB until the workbook was disposed. It now allocates 433 MB and keeps 79 MB. The tree keeps the precedents of each formula in one array instead of two sets. It holds the first formula of each precedent area without a list. It also loads the areas of each sheet into its spatial index in one pass, not one area at a time. Finding the precedents of a formula no longer allocates a list for each reference in it. Excel saves a formula filled down a column as one shared formula, and the build now parses a shared formula once for all its cells: on the same workbook saved that way, the first edit allocates 186 MB. For this, each loaded formula takes 16 bytes more memory. ([#513](https://github.com/XLibur/XLibur/issues/513))
