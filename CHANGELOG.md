...
- **One formula the parser cannot read no longer stops every edit to the workbook.** An example is `'[Book2.xlsx]Sheet1'!A1`: an external reference in the form the formula bar shows, which `FormulaA1` accepts. Once any formula in the workbook had been read, every later write anywhere in the workbook, on any sheet, threw `ExpressionParseException` about that formula, and the write had already been applied when it threw. Setting a value on that cell threw too, so only clearing its formula recovered. A save that calculates formulas wrote a formula with no cached value when it needed a full recalculation. The references in such a formula are unknown, so the calc engine now gives it none, and writes, recalculation and saves carry on. Reading the cell itself still throws `ExpressionParseException`.

- **`RecalculateAllFormulas` no longer throws on a formula that XLibur does not evaluate, or on one that the parser cannot read.** An example is the DDE formula `Sdemo123|tik!'id1?req?AAPL'`. Recalculation stopped at such a formula with `NotImplementedException`, or with `ExpressionParseException` for text the parser cannot read. A workbook that Excel opens could therefore fail to open with `LoadOptions.RecalculateAllFormulas = true`. The cell, and every formula that depends on it, is now left uncalculated (`NeedsRecalculation` stays `true`), as the cells of a circular reference are, and the rest of the workbook is calculated. Reading the cell still throws `NotImplementedException` or `ExpressionParseException`. This applies to `IXLWorkbook.RecalculateAllFormulas`, `IXLWorksheet.RecalculateAllFormulas` and recalculate-on-load. A formula that fails because of a bug in XLibur still throws. **Code that used the call to find such a formula no longer sees an exception.** Read the cells instead, or check `NeedsRecalculation` afterwards.
