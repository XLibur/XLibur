...
- **Setting a defined name to a formula the parser cannot read now throws `ArgumentException` where it used to throw `ClosedXML.Parser.ParsingException`.** `IXLDefinedName.RefersTo`, `SetRefersTo` and `IXLDefinedNames.Add` handed the dependency's own exception to the caller, which named the parser's internals rather than the argument that was wrong, and which a caller could not tell apart from a library fault. **This breaks a `catch (ParsingException)` placed around those calls** — there is no compile-time signal — so catch `ArgumentException` instead. An empty or whitespace-only formula, which used to fail the same way, is refused the same way too.
