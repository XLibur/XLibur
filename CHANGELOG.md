...
- **One unreadable defined name no longer stops a workbook from opening.** The reader passed each `<definedName>`'s text straight to the property that parses it, so a name Excel opens without complaint — an empty one, or a formula this library cannot read — aborted `new XLWorkbook(stream)` with `ClosedXML.Parser.ParsingException: Error at char 0 of ''`. Such a name is now kept with its text intact, reports `IsValid == false`, is written back unchanged, and is left alone by a shift or a sheet rename rather than being rewritten from text whose meaning was never established. This is [ClosedXML#2590](https://github.com/ClosedXML/ClosedXML/discussions/2590), whose reported triggers — `ShowTotalsRow`, `InsertRowsBelow`, `Delete(XLShiftDeletedCells.ShiftCellsLeft)` — reached the same defect from the shift side; that half was already fixed here.
