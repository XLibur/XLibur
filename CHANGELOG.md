...
- **`XLHelper.IsValidRow` no longer accepts whitespace or a leading sign**, so `IsValidRow(" 1")`, `IsValidRow("1 ")` and `IsValidRow("+1")` return `false` where they used to return `true`. It parsed with `int.TryParse`'s default number styles, which allow both. Through `IsValidA1Address` that made `$A 1`, `A1 ` and `A+1` valid addresses while `IsValidRangeAddress` rejected them — the same disagreement as above, one layer down. A row reference is digits and nothing else.
