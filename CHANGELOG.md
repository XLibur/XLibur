...
- **Renaming or deleting a sheet now rewrites the chart series that refer to it.** A series' values, its categories and the cell its name comes from kept naming the old sheet, in memory and in the saved chart part. This applied to a chart created in code and to one loaded from a file. After a rename they name the new sheet. After a delete each one reads `#REF!` where it stands. On a delete Excel also moves the series name and the categories into `c15:filtered*` extensions. XLibur does not, so the text matches Excel's and the XML does not.
