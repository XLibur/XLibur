...
- **Copying a sheet or a pivot table that Excel saved no longer throws because of a value field's base item.** Excel writes `baseField="0" baseItem="0"` on every value field, whether or not "Show values as" uses them. When field 0 had no items, as when it is the field being summed, `IXLWorksheet.CopyTo` and `IXLPivotTable.CopyTo` threw `ArgumentOutOfRangeException`, because the copy read the base item by its value. The copy now takes the base field as the position it is, because it has the same pivot cache and the same fields. It takes the base item as the same item of that field, at that item's position in the copy. The two positions can differ, because the copy lists a field's items in the pivot cache's order. A base item the field has no item for, such as Excel's `baseItem="0"` on a field with no items, is copied as it is. ([#515](https://github.com/XLibur/XLibur/issues/515))
