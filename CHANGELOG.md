...
- **A defined name that depends on its own value is now a circular reference, instead of ending the process.** A name that refers to itself, such as `Loop` defined as `=Loop+1`, or two names that refer to each other, was evaluated again and again until the stack overflowed. A stack overflow cannot be caught, and it ends the process. This happened when a cell that uses the name was read, and through `IXLWorksheet.Evaluate`. Now reading the cell, `IXLWorksheet.Evaluate` and `IXLWorkbook.Evaluate` throw `XLCircularReferenceException`, as for a cell that depends on its own value, and `IXLCell.TryGetValue` returns `false`. A name used twice in one formula, as in `=Total+Total`, is not a circular reference.
