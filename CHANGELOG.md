...
- **A sheet whose name contains a doubled apostrophe no longer loads empty.** Excel permits an apostrophe anywhere but the first and last character, so `Ann''s` is a legal sheet name — but the loader collapsed `''` to `'` before looking the sheet up, as it would for a name lifted out of a formula. The lookup then failed: the sheet loaded with its name and **none of its cells**, silently, and a workbook that also held a pivot table threw `ArgumentException: "There isn't a worksheet named …"` out of `new XLWorkbook(stream)`. Names from the file are now looked up as written.
