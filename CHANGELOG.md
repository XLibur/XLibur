...
- **An edit after a load now marks the formulas that depend on it for recalculation.** A load keeps the value Excel saved for each formula, and a read returns it until a cell the formula reads changes. But until XLibur had calculated a formula, an edit told no formula of the change. So after `ws.Cell("A2").Value = 5`, a formula `A2*10` that the file saved with the value 10 still read 10, `NeedsRecalculation` was `false`, and a save wrote 10 as its cached value. Reading any formula that had no cached value hid the problem. Now each formula that reads the edited cell is marked, whether it reads it directly, through another formula, through a defined name or from another sheet, and it reads 50. This includes shared and array formulas. It holds for these ways of changing cells: setting a cell's value or its formula, `IXLWorksheet.SetCellValue`, `InsertData`, `Clear`, `CopyTo`, `Sort` and `SortLeftToRight`, `Transpose`, and deleting every row or column of a sheet with `ws.Rows().Delete()` or `ws.Columns().Delete()`. Inserting or deleting rows, columns or cells already marked every formula in the workbook. Four of these ways marked nothing even in a workbook built in code: a sort, a transpose, `SetCellValue`, and deleting every row or column of a sheet. To find those formulas, the first edit after a load now builds XLibur's record of which formula reads which cell, as a recalculation already did. It does so once per workbook: in a workbook with 20,000 formulas, that edit takes about 180 ms longer and allocates about 100 MB, and later edits cost what they did. A formula the parser cannot read is now marked too, as the Breaking Changes entry above describes. Two faults behind it are fixed too. A formula set after that first edit was not told of later edits of the cells it read, which already happened in a workbook built in code once any formula had been read. And setting a value on a cell whose formula reads a sheet the workbook does not have, such as `Missing!A1*2`, threw `InvalidOperationException` after a recalculation. It now replaces the formula. ([#504](https://github.com/XLibur/XLibur/issues/504))
