...
public class SliceTests
{
    ...
    public void Keeps_Track_Of_Used_Rows()
    {
        ...
        Assert.That(slice.UsedRows, Is.EquivalentTo([1]));

        slice.Set(new XLSheetPoint(70, 1), 1);
        Assert.That(slice.UsedRows, Is.EquivalentTo([1, 70]));

        slice.Set(new XLSheetPoint(35, 1), 1);
        Assert.That(slice.UsedRows, Is.EquivalentTo([1, 35, 70]));
        ...
        Assert.That(slice.UsedRows, Is.EquivalentTo([70]));
    ...
    public void Keeps_Track_Of_Used_Columns()
    {
        ...
        Assert.That(slice.UsedColumns, Is.EquivalentTo([5]));

        slice.Set(new XLSheetPoint(1, 750), 1);
        Assert.That(slice.UsedColumns, Is.EquivalentTo([5, 750]));

        slice.Set(new XLSheetPoint(1, 90), 1);
        Assert.That(slice.UsedColumns, Is.EquivalentTo([5, 90, 750]));
        ...
        Assert.That(slice.UsedColumns, Is.EquivalentTo([90, 750]));

        slice.Set(new XLSheetPoint(1, 750), 0);
        Assert.That(slice.UsedColumns, Is.EquivalentTo([90]));
